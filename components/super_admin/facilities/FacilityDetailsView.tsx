// app/super-admin/facilities/[id]/FacilityDetailsView.tsx
'use client';
...
const FacilityDetailsView: React.FC<FacilityDetailsViewProps> = ({ id }) => {
  const { state, getFacilityById, updateFacilityStatus } = useSuperAdminFacility();
  ...
  const getStatusConfig = (status: FacilityStatusEnum) => {
    const configs = {
      [FacilityStatusEnum.PENDING]: {
        ...
        description: 'Awaiting approval from super admin',
        action: 'Approve Facility',
        adminMessage: 'Review and approve this facility application.',
      },
      [FacilityStatusEnum.APPROVED]: {
        ...
        label: 'Active',
        description: 'Approved and visible to users',
        action: 'Suspend Facility',
        adminMessage: 'This facility is active and operating.',
      },
  ...
  const handleStatusUpdate = async (newStatus: FacilityStatusEnum) => {
    ...
    const statusMessages = {
      [FacilityStatusEnum.APPROVED]: 'approve',
      [FacilityStatusEnum.REJECTED]: 'reject',
      [FacilityStatusEnum.SUSPENDED]: 'suspend',
      [FacilityStatusEnum.PENDING]: 'set to pending',
    };

    const action = statusMessages[newStatus];
    const confirmMessage = `Are you sure you want to ${action} this facility?\n\nFacility: ${facility.name}\nCurrent Status: ${getStatusConfig(facility.status).label}\nNew Status: ${getStatusConfig(newStatus).label}`;
  ...
  const getNextStatusAction = (currentStatus: FacilityStatusEnum) => {
    switch (currentStatus) {
      case FacilityStatusEnum.PENDING:
        return { status: FacilityStatusEnum.APPROVED, action: 'Approve', icon: CheckCircle };
      case FacilityStatusEnum.APPROVED:
        return { status: FacilityStatusEnum.SUSPENDED, action: 'Suspend', icon: Shield };
      case FacilityStatusEnum.SUSPENDED:
        return { status: FacilityStatusEnum.APPROVED, action: 'Reactivate', icon: CheckCircle };
      case FacilityStatusEnum.REJECTED:
        return { status: FacilityStatusEnum.PENDING, action: 'Review Again', icon: Clock };
      default:
        return { status: FacilityStatusEnum.APPROVED, action: 'Approve', icon: CheckCircle };
    }
  };
  ...
  const NextStatusIcon = nextStatusAction.icon;

  // Check if adminId is an object with populated data or just an ID string
  const adminInfo = facility.adminId as any;
  const isAdminPopulated = adminInfo && typeof adminInfo === 'object' && '_id' in adminInfo;
  ...
  return (
    <div className="space-y-6">
        ...
        <div className="flex items-center gap-4">
          ...
          <div>
            ...
            <div className="flex items-center gap-3 mt-1">
              <span className="text-sm text-gray-500">Super Admin View</span>
        ...
        <div className="flex items-center gap-2">
          ...
          >
            {state.loading ? (
              ...
              <NextStatusIcon className="h-4 w-4" />
            )}
            {nextStatusAction.action}
          </Button>
          <Link href={`/super-admin/services?facility=${id}`}>
      ...
      <Tabs value={activeTab} onValueChange={setActiveTab} className="w-full">
        ...
        <TabsList className="grid w-full grid-cols-2">
          ...
          <TabsTrigger value="services" className="gap-2">
            <Settings className="h-4 w-4" />
            Services ({facility.services?.length || 0})
          </TabsTrigger>
        </TabsList>
            ...
            <div className="mb-4 break-inside-avoid">
              <Card className="h-full">
                <CardHeader>
                  ...
                  <CardDescription>
                    General details and identifiers
                  </CardDescription>
                </CardHeader>
                  ...
                  <div className="space-y-3">
                    <div className="flex items-center justify-between">
                      <div className="flex items-center gap-2">
                        ...
                        <span className="text-sm font-medium text-gray-500">Facility ID</span>
                      </div>
                  ...
                  {isAdminPopulated ? (
                      ...
                      <div className="space-y-3">
                        <div className="flex items-center justify-between">
                          ...
                          <code className="text-xs font-mono bg-gray-100 px-2 py-1 rounded">
                            {adminInfo._id?.toString()?.slice(0, 8)}...
                          </code>
                        </div>
                        <div className="space-y-2">
                          <label className="text-sm font-medium text-gray-500">Name</label>
                          <p className="text-sm font-medium">{adminInfo.username || 'Not provided'}</p>
                        </div>
                        <div className="space-y-2">
                          <label className="text-sm font-medium text-gray-500">Email</label>
                          <p className="text-sm font-medium">{adminInfo.email || 'Not provided'}</p>
                        </div>
                      </div>
                      <Separator />
                      <Button
                        ...
                        asChild
                      >
                        <Link href={`/super-admin/users/${adminInfo._id}`}>
                          <Eye className="h-4 w-4" />
                          View Admin Profile
                        </Link>
                      </Button>
                    ...
                    <div className="space-y-3">
                      <div className="flex items-center justify-between">
                        <span className="text-sm font-medium text-gray-500">Admin ID</span>
                        <code className="text-xs font-mono bg-gray-100 px-2 py-1 rounded">
                          {typeof adminInfo === 'string' ? adminInfo.slice(0, 8) + '...' : 'N/A'}
                        </code>
                      </div>
                      <div className="text-center py-4">
                        <User className="h-10 w-10 text-gray-300 mx-auto mb-2" />
                        <p className="text-sm text-gray-500">Admin details not populated</p>
                        <p className="text-xs text-gray-400 mt-1">Fetch full admin info to see details</p>
                      </div>
                    </div>
                  )}
                </CardContent>
            ...
            <div className="mb-4 break-inside-avoid">
              <Card className="h-full">
                <CardHeader>
                  ...
                  <CardDescription>
                    Manage facility status and visibility
                  </CardDescription>
                </CardHeader>
                <CardContent className="space-y-4">
                  <div className="space-y-2">
                  ...
                  <div className="space-y-2">
                    ...
                    <div className="grid grid-cols-2 gap-2">
                      <Button
                        variant={facility.status === FacilityStatusEnum.APPROVED ? "default" : "outline"}
                        size="sm"
                      ...
                      <Button
                        variant={facility.status === FacilityStatusEnum.REJECTED ? "destructive" : "outline"}
                        size="sm"
                      ...
                      <Button
                        variant={facility.status === FacilityStatusEnum.SUSPENDED ? "secondary" : "outline"}
                        size="sm"
                      ...
                      <Button
                        variant={facility.status === FacilityStatusEnum.PENDING ? "secondary" : "outline"}
                        size="sm"
                        onClick={() => handleStatusUpdate(FacilityStatusEnum.PENDING)}
                        disabled={facility.status === FacilityStatusEnum.PENDING || state.loading}
                      >
                    ...
                    <p className="text-xs text-gray-500 mt-2">
                      Super Admin: You have full control over facility status
                    </p>
                  </div>
                </CardContent>
              </Card>
                  ...
                  <div className="space-y-3">
                    <div className="space-y-2">
                      ...
                      <div className="flex items-start gap-2 p-2 bg-gray-50 rounded">
                        ...
                        <p className="text-sm">{facility.location.address || 'Not provided'}</p>
                      </div>
                    </div>
                    <div className="space-y-2">
                      <label className="text-sm font-medium text-gray-500">City</label>
                      <div className="p-2 bg-gray-50 rounded">
                        <p className="text-sm">{facility.location.city || 'Not provided'}</p>
                      </div>
                    </div>
            ...
            <div className="mb-4 break-inside-avoid">
              <Card className="h-full">
                <CardHeader>
                  ...
                  <CardDescription>
                    {facility.openingHours?.filter(h => !h.isClosed).length || 0} operational days
                  </CardDescription>
                </CardHeader>
                <CardContent>
                  {facility.openingHours && facility.openingHours.length > 0 ? (
                    ...
                    <div className="text-center py-6 border-2 border-dashed border-gray-200 rounded-lg">
                      ...
                      <p className="text-sm font-medium text-gray-900">No Schedule Set</p>
                      <p className="text-xs text-gray-500 mt-1">Operating hours not configured</p>
                    </div>
                  )}
                </CardContent>
              </Card>
          ...
          <div className="mb-4 break-inside-avoid col-span-2">
            <Card className="h-full">
              <CardHeader>
                ...
                <CardDescription>
                  View facility images and videos
                </CardDescription>
              </CardHeader>
              <CardContent>
                <div className="grid grid-cols-1 lg:grid-cols-2 gap-8">
                  ...
                  <div>
                    ...
                    {facility.gallery?.images && facility.gallery.images.length > 0 ? (
                      ...
                      <div className="text-center py-8 border-2 border-dashed border-gray-300 rounded-lg">
                        ...
                        <p className="text-gray-500 mt-1">Facility has not uploaded any images</p>
                      </div>
                    )}
                  </div>
                  ...
                  <div>
                    <div className="flex items-center justify-between mb-4">
                      <div>
                        <h3 className="font-medium text-lg">Introduction Video</h3>
                        <p className="text-sm text-gray-500 mt-1">
                          Facility tour and overview
                        </p>
                      </div>
                        ...
                        <Dialog open={isVideoDialogOpen} onOpenChange={setIsVideoDialogOpen}>
                          <DialogContent className="max-w-4xl">
                            <DialogHeader>
                              ...
                              <DialogDescription>
                                {facility.name} • Super Admin View
                              </DialogDescription>
                            </DialogHeader>
                    ...
                    {facility.gallery?.introductoryVideo ? (
                      ...
                      <div className="text-center py-8 border-2 border-dashed border-gray-300 rounded-lg">
                        ...
                        <h3 className="text-lg font-medium text-gray-900">No Video Available</h3>
                        <p className="text-gray-500 mt-1">Facility has not uploaded any videos</p>
                      </div>
                    )}
                  </div>
                </div>
            ...
            <CardHeader>
              <div className="flex flex-col sm:flex-row sm:items-center sm:justify-between gap-4">
                <div>
                  ...
                  <CardDescription>
                    {facility.services?.length || 0} service(s) registered with this facility
                  </CardDescription>
                </div>
            ...
            <CardContent>
              {facility.services && facility.services.length > 0 ? (
                  ...
                  <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-4">
                    {facility.services.slice(0, 6).map((serviceId, index) => (
                      <Card key={index} className="border hover:border-blue-200 transition-colors">
                        ...
                        <CardContent>
                          <div className="space-y-3">
                            <div className="text-xs font-mono text-gray-500 bg-gray-50 px-2 py-1 rounded text-center truncate">
                              ID: {serviceId.toString()}
                            </div>
                            <Link href={`/super-admin/services/${serviceId}`} className="block">
                ...
                <div className="text-center py-12">
                  ...
                  <h3 className="text-xl font-medium text-gray-900 mb-2">No Services Registered</h3>
                  <p className="text-gray-500 mb-6 max-w-md mx-auto">
                    This facility hasn't registered any services yet.
                  </p>
                </div>
              )}
            </CardContent>
      ...
      {/* Footer Information */}
      <Card className="border-t bg-gray-50">
        <CardContent className="px-6 py-4">
          <div className="flex flex-col sm:flex-row items-start sm:items-center justify-between gap-4">
            <div className="space-y-1">
              <div className="flex items-center gap-2 text-sm text-gray-600">
                <Calendar className="h-4 w-4" />
                <span>Registered on {format(new Date(facility.createdAt), 'MMM dd, yyyy')}</span>
              </div>
            ...
            <div className="flex flex-wrap gap-2">
              <Link href={`/super-admin/bookings?facility=${id}`}>
                <Button variant="outline" size="sm" className="gap-2">
                  <Calendar className="h-4 w-4" />
                  View Bookings
                </Button>
              </Link>
              <Link href={`/super-admin/reviews?facility=${id}`}>
                <Button variant="outline" size="sm" className="gap-2">
